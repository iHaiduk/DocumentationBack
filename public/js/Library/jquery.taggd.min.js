...
(function($) {
    ...
    Taggd.prototype.on = function(event, handler) {
        ...
        this.element.off(event).on(event, handler);
    };
        ...
        $.each(this.data, function(i, v) {
            ...
            if(typeof _this.options.handlers === 'object') {
                $.each(_this.options.handlers, function(event, func) {
                    ...
                    $item.off(event).on(event, function(e) {
                        if(!handler) return;
    ...
    Taggd.prototype.updateDOM = function() {
        ...
        this.wrapper.find('span').each(function(i, e) {
            ...
            if(!$el.find("input").length){
                var text = chunk($el.text(), 20);
                $el.html(text.join("<br>"))
            }

            var left = old_left = $el.attr('data-x') * _this.element.width();
            var top = $el.attr('data-y') * _this.element.height();
            if((_this.element.offset().left + left + $el.width()/2 + 10) >= $(window).width()){
                left = $(window).width() - _this.element.offset().left - $el.width()/2 - 10;
            }
